  ...
  <table class="table">
    <thead>
      <tr>
        <th
          @click="
            onSortTypeClick({ key: 'name', sortType: SORT_TYPES.FIRST_LATTER })
          "
        >
          City
        </th>
        <th
          @click="onSortTypeClick({ key: 'min', sortType: SORT_TYPES.NUMBERS })"
        >
          MinTemp
        </th>
        <th
          @click="onSortTypeClick({ key: 'max', sortType: SORT_TYPES.NUMBERS })"
        >
          MaxTemp
        </th>
        <th
          @click="onSortTypeClick({ key: 'day', sortType: SORT_TYPES.BY_DATE })"
        >
          Day
        </th>
      </tr>
    ...
    <tbody>
      <tr v-for="(city, cityIndex) of citiesWeatherList" :key="city.id">
        ...
        <td>{{ city.min }} {{ city.temperature_unit }}</td>
        <td>{{ city.max }} {{ city.temperature_unit }}</td>
        <td>
          <Select
        ...
        <td>
          <ButtonDelete @click="onDeleteCityClick(city)"> delete </ButtonDelete>
        </td>
      </tr>
    </tbody>
  </table>
